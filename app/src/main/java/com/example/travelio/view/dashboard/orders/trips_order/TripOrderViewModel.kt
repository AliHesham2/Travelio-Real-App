...
class TripOrderViewModel(private val app: Application) : AndroidViewModel(app) {


    private var _data = MutableLiveData<List<TripReserveData>>()
    val data: LiveData<List<TripReserveData>>
        get() = _data
    ...
    private suspend fun getValidOrdersData(){
        Orders.getTripsOrdersData(1,app.resources){ data, error, success ->
            stopLoading()
            if (success){if(data != null){
                _data.value = data.data.Trips.valid_trips!!
            }
            }else{
        ...
        viewModelScope.launch(Dispatchers.IO){
            try {
                DeleteOrders.deleteTripOrder(tripReserveData.pivot.id,app.resources){ done, error ->
                    if(done){
                        _data.value = _data.value!!.minus(tripReserveData)
                    }else{
                        if(error != null){
                            whenFail(error)
                        }else{
